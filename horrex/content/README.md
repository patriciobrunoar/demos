This folder holds the real content pulled from https://www.horrex.nl/ —
copy, fonts, colors, and UI notes — so the redesign is built from actual
site content, not placeholder text.

## Workflow

1. Open https://www.horrex.nl/ in Chrome with the Claude for Chrome
   extension enabled.
...
```
Go to https://www.horrex.nl/ and do a full content + design audit of the
site so I can rebuild it. Visit every page and section you can reach from
the main navigation and footer, including any language switcher (Dutch/
English), and report back a single structured Markdown document with:
...
1. COPY — every piece of real text on the site, organized by page/section:
   ...
   - Button and CTA labels (e.g. "Request a quote", "Contact us")
   - Footer content: links, legal text, addresses, social links
   - Contact info: phone, email, physical address, opening hours, any
     contact form fields and their labels
   - Any product/service names, categories, and descriptions
   - Testimonials, certifications, partner/client logos mentioned by name
...
2. TYPOGRAPHY — inspect computed styles (DevTools) for headings and body
   ...
   - Letter-spacing / text-transform if used (e.g. uppercase nav links)
...
4. LOGOTYPE & IMAGERY:
   ...
   - List every meaningful image on the site with its URL and alt text
     (hero images, product photos, team photos, icons)
...
6. SITE STRUCTURE:
   ...
   - Any forms (contact, quote request, newsletter) and their exact fields
